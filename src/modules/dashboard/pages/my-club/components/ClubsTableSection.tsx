      ...
      <div className="flex items-center gap-2">
        <div className="relative">
          ...
          <select
            value={statusFilter}
            onChange={(event) =>
              onStatusFilterChange(event.target.value as ClubStatus | "all")
            }
                ...
                >
                  ...
                  <td className="whitespace-nowrap rounded-r-2xl bg-white p-4 shadow-sm shadow-slate-200/50 ring-1 ring-slate-100 text-right align-middle">
                    <div className="flex items-center justify-end gap-4">
                      {/* Status Badge */}
                      <div
                        className={`hidden sm:flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-semibold ${statusMeta.className}`}
                      >
                        <span
                          className={`h-1.5 w-1.5 rounded-full ${statusMeta.iconColor}`}
                        />
                        {statusMeta.label}
                      </div>
      ...
      <div className="flex flex-col items-center justify-between gap-4 py-2 sm:flex-row">
        <p className="text-xs font-medium text-slate-500">
          ...
          <span className="text-slate-900">
            ...
            {Math.min(filteredCount, currentPage * rowsPerPage)}
          </span>{" "}
          / {filteredCount}
        </p>
